...
# Author: PWB5
# GitHub: https://github.com/PWB5/DSTServer
#--------------------------------------------------------------------------------
#需配合远程连接软件（推荐Xshell和Xftp，或putty和winscp）使用
#首次使用时，上传脚本至用户目录并给予执行权限
#云服务器系统建议安装ubuntu系统(linux发行版本)
#V1.0.1 19/02/19
# 原版
#V1.0.2	19/02/19
...
#V1.1.0 19/07/02
# 增强“ServerStatus”函数：显示DST服务器中的的玩家编号、ID、用户名
# 优化“ZipSave”函数：压缩存档添加修改的时间戳，并移动到$HOME/.klei/backup/SaveZip
#V1.1.1	19/07/05
# 优化“ServerStatus”函数：增加“ | grep "\] (KU_"”
# 增强“unzipSave”函数：显示“${HOME}目录下现有的存档zip文件名：”
# 优化“deldir”函数：删除前关闭服务器
#-------------------------------------------------------------------------------
# 脚本变量
	...
	do
		...
		if [[  "$Savelist" == "" ]]; then
			...
			if [[ "$clustername" == "" ]]; then
				...
				if [[ "$RunSaveName" == "$clustername" ]]; then
					if [[ $(screen -ls | grep -c "DST_Master") > 0 || $(screen -ls | grep -c "DST_Caves") > 0 ]]; then
						closeserver
...
function ServerStatus()
{
	if [[ $(screen -ls | grep -c "DST_Master") > 0 || $(screen -ls | grep -c "DST_Caves") > 0 ]]; then
		...
		echo -e "${C_YELLOW}正在运行的存档名：${C_GREEN}${RunSaveName}	${C_YELLOW}服务器名：${C_GREEN}${RunServerName}\e[0m"
		if [[ $(screen -ls | grep -c "DST_Master") > 0 ]]; then
			echo -ne "${C_YELLOW}Master : \e[0m"
			screen -S "DST_Master" -p 0 -X stuff "c_listallplayers()$(printf \\r)"
			sleep 0.03
			MasterPlayersTemp=$(grep -E "c_listallplayers" "${DST_conf_basedir}/${DST_conf_dirname}/${RunSaveName}/Master/server_log.txt"  | tail -1 | cut -d[ -f2 | cut -d] -f1)
		    MasterPlayers=$(grep "\[${MasterPlayersTemp}\]: \[" "${DST_conf_basedir}/${DST_conf_dirname}/${RunSaveName}/Master/server_log.txt" | grep "\] (KU_" | cut -d' ' -f2-5)
		    if [[ "$MasterPlayers" != "" ]]; then
		    	echo -e "${C_GREEN}\n${MasterPlayers}\e[0m"
		    else
		    	echo -e "${C_YELLOW}没有玩家在线\e[0m"
		    fi
		else
			echo -e "${C_RED}Master未开启！\e[0m"
		fi
		if [[ $(screen -ls | grep -c "DST_Caves") > 0 ]]; then
			echo -ne "${C_YELLOW}Caves : \e[0m"
			screen -S "DST_Caves" -p 0 -X stuff "c_listallplayers()$(printf \\r)"
			sleep 0.03
			CavesPlayersTemp=$(grep -E "c_listallplayers" "${DST_conf_basedir}/${DST_conf_dirname}/${RunSaveName}/Caves/server_log.txt"  | tail -1 | cut -d[ -f2 | cut -d] -f1)
		    CavesPlayers=$(grep "\[${CavesPlayersTemp}\]: \[" "${DST_conf_basedir}/${DST_conf_dirname}/${RunSaveName}/Caves/server_log.txt" | grep "\] (KU_" | cut -d' ' -f2-5)
		    if [[ "$CavesPlayers" != "" ]]; then
		    	echo -e "${C_GREEN}\n${CavesPlayers}\e[0m"
		    else
		    	echo -e "${C_YELLOW}没有玩家在线\e[0m"
		    fi
		else
			echo -e "${C_RED}Caves未开启！\e[0m"
		fi
		
	else
		echo -e "${C_RED}DST_Server未开启！\e[0m"
	fi
}
# 保存并关闭DST服务器
...
function unzipSave()
{
	...
	cd $HOME
	AllSaveName=$(ls -l $HOME | grep .zip | cut -d':' -f2 | cut -d' ' -f2 | cut -d'.' -f1)
	echo -e "${C_YELLOW}${HOME}目录下现有的存档zip文件名：\e[0m"
	echo -e "$AllSaveName"
	echo -e "${C_YELLOW}请输入要解压的存档zip文件名：\e[0m"
	read NameSave
...
function ZipSave()
{
	...
	if [[  "$Savelist" == "" ]]; then
		...
		if [[ "$Savelist" == *"$clustername"* ]]; then
			if [[ "$(echo "$clustername" | grep "$Savelist")" != "" ]]; then
				LastChangeTime=$(stat -c %Y $DST_conf_basedir/$DST_conf_dirname/$clustername/Master/save/saveindex)
				if [[ ! -d ${DST_conf_basedir}/backup/SaveZip ]]; then
					mkdir ${DST_conf_basedir}/backup/SaveZip
				fi
				#rm -rf ${clustername}.zip
				zip -qr ${clustername}-${LastChangeTime}.zip $clustername
				mv -u ${clustername}-${LastChangeTime}.zip ${DST_conf_basedir}/backup/SaveZip
				cd $HOME
...
function menu()
{
    ...
    do
		...
		echo -e "${C_BLUE}[13]解压存档zip        [14]解压Mods.zip       [15]压缩存档           [16]更改令牌           \e[0m"
				ServerStatus
        echo -e "${C_YELLOW}============================19/06/29 V1.1.1 更多功能开发中...============================\e[0m"
