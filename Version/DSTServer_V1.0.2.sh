...
# 初版
#V1.0.2	19/02/19
# 拆分出“安装DST服务器”、“新建存档”、“设置管理员”、“设置黑白名单”功能
#-------------------------------------------------------------------------------
...
{
	# if [ ! -d "./steamcmd" ];then
		echo -e "\e[33m必要的Linux软件安装中...\e[0m"
		...
		echo -e "${C_GREEN}必要的Linux软件安装完成！\e[0m"
		echo -e "\e[33mSteamcmd安装中...\e[0m"
		mkdir ./steamcmd
        cd ./steamcmd
		...
		cd $HOME
		echo -e "${C_GREEN}Steamcmd安装完成！\e[0m"
	# fi
    echo -e "\e[33mDST_Server安装中...\e[0m"
	cd ./steamcmd	
	...
	cd $HOME
	echo -e "${C_GREEN}DST_Server安装完成！\e[0m"
	echo -e "\e[33m虚拟内存开启中...\e[0m"
	sudo dd if=/dev/zero of=/swapfile bs=1M count=2048
    sudo mkswap /swapfile
    sudo swapon /swapfile
	sudo chmod 0646 /etc/fstab
    echo "/swapfile swap swap defaults 0 0" >> /etc/fstab
    echo -e "${C_GREEN}虚拟内存开启完成！\e[0m"
}
# 安装并执行DST_Server自动更新程序
...
{
	if [[ $(screen -ls | grep -c "DST_Master") > 0 || $(screen -ls | grep -c "DST_Caves") > 0 ]]; then
	    echo -e "${C_BLUE}即将跳转游戏服务器窗口，要退回本界面，在游戏服务器窗口按“ctrl+a”+“d” 再执行脚本即可。\e[0m"
		sleep 3
	    if [[ $(screen -ls | grep -c "DST_Master") > 0 && $(screen -ls | grep -c "DST_Caves") > 0 ]]; then
	        screen -r DST_Master
	    fi
...
function setworld()
{
	echo -e "${C_BLUE}请选择要更改的地上世界设置：（当前服务器不开地上直接选默认）\e[0m"
...
function dellist()
{
	...
	do
		...
		read kleiid
		if [[ "$kleiid" == "0" ]]; then
		    echo -e "${C_BLUE}删除完毕！\e[0m"
			break
		else
			if [[ $(grep "$kleiid" -c "${DST_conf_basedir}/${DST_conf_dirname}/${cluster_name}/$listfile") > 0 ]] ;then 
				sed -i "/$kleiid/d" ${DST_conf_basedir}/${DST_conf_dirname}/${cluster_name}/$listfile
				echo -e "${C_GREEN}名单已移除！\e[0m"
			else
			    echo -e "${C_YELLOW}名单不存在！\e[0m"
			fi
		fi
	done
}
...
# 创建并设置Mods，默认添加“中文语言包（服务器版）”Mod
function setupmod()
{
    echo "ServerModSetup(\"1301033176\")" > "${DST_conf_basedir}/${DST_conf_dirname}/mods_setup.lua"
    echo "return {
	...
	do
		...
		if [[ "$modid" == "0" ]]; then
			...
			then 
				...
				echo -e "${C_GREEN}地上世界Mod移除完成\e[0m"
			else
			...
			then 
				...
				echo -e "${C_GREEN}洞穴世界Mod移除完成\e[0m"
			else
...
{
    if [[ $(screen -ls | grep -c "DST_Master") > 0 || $(screen -ls | grep -c "DST_Caves") > 0 ]]; then
    	echo -e "\e[34mDST_Server关闭中...\e[0m"
	    if [[ $(screen -ls | grep -c "DST_Master") > 0 ]]; then
            screen -S "DST_Master" -p 0 -X stuff "c_shutdown(true)$(printf \\r)"
	    fi
	    if [[ $(screen -ls | grep -c "DST_Caves") > 0 ]]; then
	    ...
	    sleep 1
	    echo -e "${C_GREEN}DST_Server已关闭！\e[0m"
	else
...
function menu()
{
    ...
    do
		...
		echo -e "${C_BLUE}首次使用请将本地电脑上的MOD上传到云服务器$HOME/Steam/steamapps/common/DST_Public/mods目录下\e[0m"
		echo
        echo -e "${C_BLUE}[01]安装DST服务器      [02]启动服务器         [03]关闭服务器         [04]重启服务器         \e[0m"
        echo -e "${C_BLUE}[05]查看自动更新进程   [06]查看游戏服务器状态 [07]新建存档           [08]删除存档           \e[0m"
		echo -e "${C_BLUE}[09]添加或移除MOD      [10]设置管理员         [11]设置黑白名单       [12]退出本脚本         \e[0m"
        echo -e "${C_YELLOW}============================================================================================\e[0m"
        ...
        read cmd
		    case $cmd in
                ...
                8)
				    ...
				    break;;	
                9)
	                echo -e "${C_BLUE}设置完成后，须重启服务器才会生效。\e[0m"
					echo -e "${C_BLUE}已有存档：\e[0m"
		            ls -l ${DST_conf_basedir}/${DST_conf_dirname} |awk '/^d/ {print $NF}'
