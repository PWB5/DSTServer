...
function ServerStatus()
{
	if [[ $(screen -ls | grep -c "DST_Master") > 0 || $(screen -ls | grep -c "DST_Caves") > 0 ]]; then
		...
		DST_now=$(date +"%D %T")
		echo -e "${C_YELLOW}[${DST_now}] 正在运行的存档名：${C_GREEN}${RunSaveName}	${C_YELLOW}服务器名：${C_GREEN}${RunServerName}	\e[0m"
		if [[ $(screen -ls | grep -c "DST_Master") > 0 ]]; then
		    ...
		    MasterPlayers=$(grep "\[${MasterPlayersTemp}\]: \[" "${DST_conf_basedir}/${DST_conf_dirname}/${RunSaveName}/Master/server_log.txt" | cut -d' ' -f2-5)
...
function menu()
{
    ...
    do
		...
		echo -e "${C_BLUE}[13]解压存档zip        [14]解压Mods.zip       [15]压缩存档           [16]更改令牌           \e[0m"
				ServerStatus
        echo -e "${C_YELLOW}============================19/06/29 V1.1.0 更多功能开发中...============================\e[0m"
