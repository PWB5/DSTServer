...
function setworld()
{
	...
	read masterset
	# while [[ masterset<1 && masterset>5 ]]; do
	# 	echo -e "${C_RED}请输入数字（1~5）选择地上世界设置类型\e[0m"
	# 	read masterset
	# done
	case $masterset in
	    1)
...
function unzipSave()
{
	...
	cd $HOME
	unzip -oq Save.zip -d ${DST_conf_basedir}/${DST_conf_dirname}
	echo -e "${C_GREEN}Save.zip解压完毕！\e[0m"
}
# 自动解压Mods.zip到相应位置
function unzipMods()
{
