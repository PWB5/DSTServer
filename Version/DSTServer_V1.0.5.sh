...
#V1.0.5
# 优化“InstallServer”逻辑：虚拟内存先关闭，再设置，后开启（默认4GB）
# 重构“startcheck”函数
#-------------------------------------------------------------------------------
# 脚本变量
...
function savelog()
{
    ...
    if [ -f "${DST_conf_basedir}/${DST_conf_dirname}/${cluster_name}/Master/server_chat_log.txt" ]; then
		echo -e "${C_YELLOW}保存服务器消息日志。\e[0m"
		DST_now=$(date +"%D %T")
...
function settoken()
{
	...
	do
		...
		if [[ "$token" == "" ]]; then
			...
			echo -e "${C_RED}输入的服务器令牌格式错误！请重新输入：（按Enter键结束）\e[0m"
		fi
	done
}
# 设置房间基本参数
...
function startcheck()
{
	...
	do
		if [[ $(screen -ls | grep -c "DST_Master") = 0 ]]; then
			echo -e "${C_RED}DST_Master服务器未开启!\e[0m"
			if [[ $(screen -ls | grep -c "DST_Caves") = 0 ]]; then
				echo -e "${C_RED}DST_Caves服务器未开启!"
				echo -e "请执行关闭服务器命令后再次尝试，并注意令牌是否成功设置且有效。\e[0m"
				break
			else
				for ((a=1; a<=30; a++))
				do
					if [[ $(grep "Registering slave in China lobby" -c "${DST_conf_basedir}/${DST_conf_dirname}/${cluster_name}/Caves/server_log.txt") > 0 ]]; then
						echo -e "${C_GREEN}DST_Caves服务器开启成功，和小伙伴尽情玩耍吧！\e[0m"
						break
						...
						echo -e "${C_YELLOW}SteamGameServer初始化成功！\e[0m"
						CState=4
			...
			for ((b=1; b<=30; b++))
			do
				if [[ $(grep "Registering master server in China lobby" -c "${DST_conf_basedir}/${DST_conf_dirname}/${cluster_name}/Master/server_log.txt") > 0 ]]; then
					echo -e "${C_GREEN}服务器开启成功，和小伙伴尽情玩耍吧！\e[0m"
					break
 ...
 #        if [[ $(grep "Your Server Will Not Start !!!" -c "${DST_conf_basedir}/${DST_conf_dirname}/${cluster_name}/Master/server_chat_log.txt") > 0 ]]; then
	# 	    echo "${C_RED}服务器开启未成功，请执行关闭服务器命令后再次尝试，并注意令牌是否成功设置且有效。\e[0m"
	#     else
	# 	    echo -e "${C_GREEN}服务器开启成功，和小伙伴尽情玩耍吧！\e[0m"
	# 	fi
	# else
...
{
    if [[ $(screen -ls | grep -c "DST_Master") > 0 || $(screen -ls | grep -c "DST_Caves") > 0 ]]; then
    	echo -e "${C_YELLOW}DST_Server关闭中...\e[0m"
	    if [[ $(screen -ls | grep -c "DST_Master") > 0 ]]; then
            screen -S "DST_Master" -p 0 -X stuff "c_shutdown(true)$(printf \\r)"
	    fi
	    if [[ $(screen -ls | grep -c "DST_Caves") > 0 ]]; then
...
function startserver()
{
	...
	cd $HOME/Steam/steamapps/common/DST_${DST_game_beta}/bin
	screen -dmS \"DST_Master\" /bin/sh -c \"$DST_bin_cmd -console -conf_dir $DST_conf_dirname -cluster $cluster_name -shard Master\"
	echo -e \"\e[33m正在重启服务器...请稍后...\e[0m\"
	sleep 15" > $HOME/rebootserver.sh
        echo 'for ((i=1; i<=60; i++))
	do
	if [[ $(screen -ls | grep -c "DST_Master") = 0 && $(screen -ls | grep -c "DST_Caves") > 0 ]]; then
	    if [[ $(grep "Your Server Will Not Start !!!" -c "$masterchatlog_path") > 0 ]]; then
	    	if [[ i = 1 ]]; then
	    		echo "\e[33m服务器开启中...\e[0m"
	    	elif [[ i > 1 && i < 60 ]]; then
	    		echo -e ".\c"
	    	else 
	    		echo "\e[31m服务器开启未成功，请执行关闭服务器命令后再次尝试，并注意令牌是否成功设置且有效。\e[0m"
				break
	    	fi
		else
			echo -e "\e[92m服务器开启成功，和小伙伴尽情玩耍吧！\e[0m"
			break
	    fi
	else
		if [[ $(screen -ls | grep -c "DST_Master") > 0 ]]; then
	...
	echo -e "${C_YELLOW}服务器开启中...请稍候...\e[0m"
	sleep 5
    auto_update_process
	sleep 10
	startcheck
...
function menu()
{
    ...
    do
		...
		# echo -e "${C_BLUE}首次使用请将本地电脑上的MOD上传到云服务器目录下\e[0m"
		...
		echo -e "${C_BLUE}[13]解压Save.zip       [14]解压Mods.zip       [15]开发中...\e[0m"
