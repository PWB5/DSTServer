	...
	if [[ $(screen -ls | grep -c "DST_Master") == 0 ]]; then
		...
		do
			if [[ $MState == 4 && $(grep "Registering master server in China lobby" -c "${DST_conf_basedir}/${DST_conf_dirname}/${cluster_name}/Master/server_log.txt") > 0 ]]; then
				echo -e "${C_GREEN}DST_Master服务器开启成功，和小伙伴尽情玩耍吧！\e[0m"
				MState=5
				break
		...
		## Master开启成功，判断Caves是否开启成功
		if [[ $(screen -ls | grep -c "DST_Caves") == 0 ]]; then
			echo -e "${C_YELLOW}DST_Caves服务器未开启!\e[0m"
		else
			## 判断DST_Caves服务器是否开启
			for ((c=1; c<=30; c++))
			do
...
function menu()
{
    ...
    do
		...
		echo -e "${C_BLUE}[13]解压Save.zip       [14]解压Mods.zip       [15]压缩存档           [16]更改令牌           \e[0m"
        echo -e "${C_YELLOW}======================================更多功能开发中======================================\e[0m"
